jQuery(function($){
	...
	function FeedbackList($block){
		...
		var currentType = pro.enable_types ? 'idea' : null;
		...
		function loadItems(removeExisting){
			...
			$.get(pro.ajaxurl, 
				{action: 'un_get_feedback', page: nextPage, type: currentType}, function(response){
	...
	function Discussion(){
		...
		function feedbackSelectedHandler(event, itemId){
			...
			$listWrapper.hide(); 
			$list.html('');
			$.post(pro.ajaxurl, {action: 'un_get_comments', post_id: itemId}, commentsLoadedHandler);
		}
